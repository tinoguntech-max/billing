...
const styles = [
  { grad: 'from-pastel-purple to-pastel-blue',   icon: Zap,    accent: 'text-accent-purple' },
  { grad: 'from-pastel-mint to-pastel-sky',       icon: Rocket, accent: 'text-accent-blue'   },
  { grad: 'from-pastel-peach to-pastel-yellow',   icon: Flame,  accent: 'text-accent-peach'  },
  { grad: 'from-pastel-pink to-pastel-purple',    icon: Crown,  accent: 'text-accent-pink'   },
]
...
export default function Paket() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const r = await apiFetch('/api/paket')
    setData(await r.json()); setLoading(false)
  }, [])
  useEffect(() => { load() }, [load])

  const openAdd  = () => { setForm({ ...emptyForm }); setEditId(null); setModal(true) }
  const openEdit = p => { setForm({ nama_paket: p.nama_paket, kecepatan: String(p.kecepatan), harga: String(p.harga), deskripsi: p.deskripsi || '' }); setEditId(p.id); setModal(true) }
  const save = async () => {
    if (!form.nama_paket || !form.kecepatan || !form.harga) { setToast({ msg: 'Semua field wajib!', type: 'error' }); return }
    const r = await fetch(editId ? `/api/paket/${editId}` : '/api/paket', {
  ...
  const hapus = async id => {
    ...
    setToast({ msg: 'Paket dihapus', type: 'success' }); load()
  }
    ...
    <div>
      ...
      {loading
        ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => <div key={i} className="card p-5 animate-pulse"><div className="w-10 h-10 rounded-xl bg-gray-100 mb-3" /><div className="h-5 bg-gray-100 rounded mb-2 w-3/4" /><div className="h-4 bg-gray-100 rounded w-1/2 mb-3" /><div className="h-8 bg-gray-100 rounded" /></div>)}
          </div>
        : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {data.map((p, i) => {
                  ...
                  <div className="p-5">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex gap-1">
                        <button onClick={() => openEdit(p)} className="w-7 h-7 rounded-lg bg-pastel-blue flex items-center justify-center text-accent-blue hover:bg-blue-100 text-xs"></button>
                        <button onClick={() => hapus(p.id)} className="w-7 h-7 rounded-lg bg-pastel-pink flex items-center justify-center text-accent-pink hover:bg-pink-100"><X size={12} /></button>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-purple-50 pt-3 flex items-center justify-between">
                      <span className="text-accent-purple font-bold text-sm">{fmt(p.harga)}<span className="text-muted font-normal text-xs">/bln</span></span>
            ...
            <div onClick={openAdd} className="border-2 border-dashed border-purple-200 rounded-2xl flex flex-col items-center justify-center p-8 cursor-pointer hover:bg-purple-50 transition-colors min-h-[180px] bg-pastel-lavender">
              <div className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center mb-3 shadow-soft"><Plus size={20} className="text-accent-purple" /></div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editId ? 'Edit Paket' : 'Tambah Paket Internet'} headerColor="bg-pastel-peach">
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <textarea className="input-field h-20 resize-none" value={form.deskripsi} onChange={e => setForm({ ...form, deskripsi: e.target.value })} />
          </div>
        </div>
        <div className="px-6 pb-6 flex gap-3">
          ...
          <button onClick={save} className="flex-1 py-2.5 rounded-xl btn-submit justify-center">Simpan Paket</button>
        </div>
      </Modal>
    </div>
  )
