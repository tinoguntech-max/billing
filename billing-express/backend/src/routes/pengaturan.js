...
router.post('/', upload.single('logo_file'), async (req, res) => {
  try {
    const { nama_isp, telepon, email, website, alamat, mikrotik_host, mikrotik_user, mikrotik_password, mikrotik_port } = req.body
    ...
    if (existing) {
      await pool.query(
        `UPDATE pengaturan SET nama_isp=?,telepon=?,email=?,website=?,alamat=?,logo_url=COALESCE(?,logo_url),
         mikrotik_host=?,mikrotik_user=?,mikrotik_password=?,mikrotik_port=? WHERE id=?`,
        [nama_isp, telepon, email, website, alamat, logo_url, mikrotik_host, mikrotik_user, mikrotik_password, mikrotik_port || 8728, existing.id]
      )
    } else {
      await pool.query(
        `INSERT INTO pengaturan (nama_isp,telepon,email,website,alamat,logo_url,mikrotik_host,mikrotik_user,mikrotik_password,mikrotik_port)
         VALUES (?,?,?,?,?,?,?,?,?,?)`,
