...
// Create directories
const directories = [
  path.join(baseDir, 'src', 'app', 'pengeluaran'),
  path.join(baseDir, 'src', 'app', 'api', 'pengeluaran')
];
...
const files = [
  { path: path.join(baseDir, 'src', 'app', 'pengeluaran', 'page.tsx'), content: pageContent },
  { path: path.join(baseDir, 'src', 'app', 'api', 'pengeluaran', 'route.ts'), content: routeContent }
];
...
files.forEach(file => {
  try {
    ...
    console.log(`✓ Created: ${file.path} (${size} bytes)`);
  } catch (err) {
...
console.log('\nFiles:');
files.forEach(file => {
  const exists = fs.existsSync(file.path) && fs.statSync(file.path).isFile();
  const size = exists ? fs.statSync(file.path).size : 0;
  console.log(`${exists ? '✓' : '✗'} ${path.relative(baseDir, file.path)} (${size} bytes)`);
  allOk = allOk && exists;
});

console.log('\n' + '='.repeat(60));
if (allOk) {
  console.log('✓ SUCCESS: All 4 items created successfully!');
  console.log('='.repeat(60));
  console.log('Summary:');
  console.log('  Directories: 2');
  console.log('    1. src\\app\\pengeluaran');
  console.log('    2. src\\app\\api\\pengeluaran');
  console.log('  Files: 2');
  console.log('    3. src\\app\\pengeluaran\\page.tsx');
  console.log('    4. src\\app\\api\\pengeluaran\\route.ts');
  console.log('='.repeat(60));
  process.exit(0);
} else {
  console.log('✗ FAILURE: Some items were not created!');
  console.log('='.repeat(60));
  process.exit(1);
}
