...
// Create directories
const dirsToCreate = [
    path.join(basePath, 'src', 'app', 'pengeluaran'),
    path.join(basePath, 'src', 'app', 'api', 'pengeluaran')
];

console.log('Creating pengeluaran directories...');
for (const dirPath of dirsToCreate) {
    try {
        ...
        console.log(`✗ Error creating ${dirPath}: ${e.message}`);
    }
}

// Create files
const filesToCreate = {
    [path.join(basePath, 'src', 'app', 'pengeluaran', 'page.tsx')]: `'use client'
...
`,
    [path.join(basePath, 'src', 'app', 'api', 'pengeluaran', 'route.ts')]: `import { NextRequest, NextResponse } from 'next/server'
...
for (const [filePath, content] of Object.entries(filesToCreate)) {
    try {
        ...
        console.log(`✗ Error creating ${filePath}: ${e.message}`);
    }
}
